...
public class AuthHelper {
	...
	public void setRedisTemplate(RedisTemplate<String, String> redisTemplate) {  
		AuthHelper.redisTemplate = redisTemplate;  
	}  
    ...
    public static String getAccessToken() throws OApiException {
    	...
    	String json = value.get("accesstoken");
    	JSONObject accessTokenValue = null;
    	if (StringUtils.isNotBlank(json)) {
    		accessTokenValue = JSONObject.parseObject(json);
		}
        long curTime = System.currentTimeMillis();
//        JSONObject accessTokenValue = null;// (JSONObject) FileUtils.getValue("accesstoken", Env.CORP_ID);
        String accToken = "";
//        System.out.println(accessTokenValue);
        if (accessTokenValue == null || curTime - accessTokenValue.getLong("begin_time") >= cacheTime) {
            try {
                ...
                jsonAccess.put("access_token", accToken);
                jsonAccess.put("begin_time", curTime);
                //真实项目中最好保存到数据库中
//                FileUtils.write2File(jsonAccess, "accesstoken");
                value.set("accesstoken", JSONObject.toJSONString(jsonAccess));
    ...
    public static String getJsapiTicket(String accessToken) throws OApiException {
    	//改造SDK 使用redis存储
    	ValueOperations<String, String> value = redisTemplate.opsForValue();
    	String json = value.get("jsticket");
    	JSONObject jsTicketValue = null ;
    	if(StringUtils.isNotBlank(json)) {
    		jsTicketValue = JSONObject.parseObject(json);
    	}
...
//        JSONObject jsTicketValue = (JSONObject) FileUtils.getValue("jsticket", Env.CORP_ID);
        ...
        if (jsTicketValue == null || curTime -
            ...
            try {
                ...
                jsonTicket.put("ticket", jsTicket);
                jsonTicket.put("begin_time", curTime);
//                FileUtils.write2File(jsonTicket, "jsticket");
                value.set("ticket", JSONObject.toJSONString(jsonTicket));
            } catch (Exception e) {
                e.printStackTrace();
            }
            return jsTicket;
     ...
     * @return
     */
	public static DingSignatureResp getConfig(String url) {
...
//        String agentid = null;
        ...
        DingSignatureResp resp = new DingSignatureResp();
        resp.setAgentId(Env.AGENT_ID);
        resp.setCorpId(Env.CORP_ID);
        resp.setNonstr(nonceStr);
